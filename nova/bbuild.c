...
void nova_init_aio_struct(struct nova_inode_info_header *sih){
	struct nova_inode_info *ino_info= container_of(sih,struct nova_inode_info,header);
	ino_info->aio.wait_bitmap = ino_info->aio.work_bitmap = NULL;
